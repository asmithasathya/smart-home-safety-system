...
### Rolled Back UI Extras

- Reverted the browser dashboard, visual command center server, distributed UART agent, and demo script so the repo is back to the core controller, light, and alarm project.
- Kept the controller, light, and alarm firmware directories unchanged during the rollback.
- Updated `README.md` so the current status still accurately lists all three implemented board apps.
